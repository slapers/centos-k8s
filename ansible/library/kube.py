...
class KubeManager(object):
    ...
    def __init__(self, module):
        ...
        self.update_period = module.params.get('update_period')
    ...
    def rolling_update(self):
        ...
        if not self.name:
            if self.label:
                ...
                self.module.fail_json(msg='Unable to find replication controller without label or name')
        else:
            cmd.append(self.name)
                        
        name = self._execute_nofail(cmd)
        if not name:
            self.module.fail_json(msg='No existing replication controller found')
        if len(name) != 1:
            self.module.fail_json(msg='Unexpected amount of replication controllers found')
        name = name[0].split()[0]

        if not self.filename:
            self.module.fail_json(msg='Need a filename parameter to perform rolling update')
        
        cmd = ['rolling-update']
        cmd.append(name)        
        cmd.append('--filename=' + self.filename)
        cmd.append('--update-period=' + self.update_period)
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            update_period=dict(default='1m'),
            state=dict(default='present', choices=['present', 'absent', 'latest', 'reloaded', 'stopped', 'rolling-update']),
            )
        )
